...
document.addEventListener('init', function (event) {
    ...
    if (page.id === 'addressPage') {
      ...
      $("#setAdd").click(function () {
        ons.notification.alert("Delivery "+latitude+' '+longitude);
...
if (page.id === 'orderPage') {
    ...
    if(lat === null){
        ...
        price = 0
        localStorage.clear();
        ons.notification.alert("Please set location")
        $("#content")[0].load("address.html");
    }
    var address = "Latitude : "+lat+" Longtigude : "+long
    $('#showLatLong').append(address)
    order.forEach((item, index) => {
...
$("#pay").click(function () {
    order = []
    price = 0
    localStorage.setItem("nameRes",null);
    localStorage.setItem("resPic",null);
